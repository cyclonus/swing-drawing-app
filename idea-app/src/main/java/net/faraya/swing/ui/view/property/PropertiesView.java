 ...
 * @author  tom
 */
public class PropertiesView extends JPanel implements TableModelListener
{
    ...
    public PropertiesView( Object draw_object )
    {
    ...
    private Vector getListEntries()
    {
        ...
        for( int i = 0; i < the_methods.length; i++ )
        {
            ...
            if( method_string.indexOf("setProperty") != -1 )
            {
                ...
                try
                {
                    Method get_method  = c.getMethod( "getProperty"+method_name );
                    set_methods_[get_count++] = the_methods[i];
                    tmp_vector.add( method_name );
                    tmp_vector.add( get_method.invoke( draw_object_ ) );
                }
